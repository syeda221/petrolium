...
<head>
  ...
  <title>Other Income - {{ $income->id }}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .voucher-title { font-size: 13px; font-weight: 700; color: #0ea5e9; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 4px; }
    ...
    .amount-row .val { font-size: 20px; color: #0ea5e9; }
  ...
  <div class="voucher">
    ...
    <div class="v-body">
      ...
      <table class="info-table">
        <tr><td class="lbl">Voucher No</td><td class="val">INC-{{ str_pad($income->id, 4, '0', STR_PAD_LEFT) }}</td></tr>
        <tr><td class="lbl">Date</td><td class="val">{{ \Carbon\Carbon::parse($income->date)->format('d-M-Y') }}</td></tr>
        <tr><td class="lbl">Source</td><td class="val">{{ $income->title }}</td></tr>
        <tr>
          <td class="lbl">Deposit To</td>
          <td class="val">
            @if($income->party_type == 'account'){{ $income->account->title ?? '-' }}
            @elseif($income->party_type == 'vendor'){{ $income->vendor->name ?? '-' }}
            @else{{ $income->customer->customer_name ?? '-' }}
            @endif
            ({{ ucfirst($income->party_type) }})
          </td>
        </tr>
        <tr class="amount-row"><td class="lbl">Amount</td><td class="val">Rs. {{ number_format($income->amount, 2) }}</td></tr>
        <tr><td class="lbl">Remarks</td><td class="val">{{ $income->remarks ?? '-' }}</td></tr>
      </table>
      <hr class="divider">
