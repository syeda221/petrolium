...
<div class="main-content">
    <div class="container-fluid px-3 px-md-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h3 class="fw-bold mb-1" style="color:var(--voucher-text);">Edit Income Voucher</h3>
                <p class="text-muted mb-0" style="font-size:13px;">Update other income details</p>
            </div>
        ...
        <div class="voucher-form-card">
            ...
            @endif
            <form action="{{ route('other.income.update', $income->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-5">
                        <label class="form-label">Source / Title <span class="text-danger">*</span></label>
                        <input type="text" name="title" class="form-control" value="{{ $income->title }}" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Amount <span class="text-danger">*</span></label>
                        <input type="number" name="amount" class="form-control" step="0.01" min="1" value="{{ $income->amount }}" required>
                    </div>
                </div>

                <div class="section-card">
                    <h6 class="sub-heading sub-heading-primary"><i class="bi bi-person-badge"></i> Deposit To</h6>
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label">Party Type</label>
                            <input type="text" class="form-control" value="{{ ucfirst($income->party_type) }}" readonly>
                        </div>
                        <div class="col-md-8">
                            <label class="form-label">Party / Account</label>
                            <input type="text" class="form-control" value="{{ $income->party_type == 'account' ? ($income->account->title ?? '') : ($income->party_type == 'vendor' ? ($income->vendor->name ?? '') : ($income->customer->customer_name ?? '')) }}" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-voucher">
                        <i class="bi bi-check2-circle me-1"></i> Update Income
                    </button>
                </div>
            </form>
        </div>
