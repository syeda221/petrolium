...
<body>
  ...
  <div class="receipt" id="receipt">
    ...
    <table>
      <tr>
        <td style="width:60%; font-weight:700;">@if(is_numeric($voucher->type)) Account
        @elseif($voucher->type === 'vendor') Vendor @elseif($voucher->type === 'customer') Customer @else Party @endif
        </td>
        ...
        @foreach($rows as $i => $row)
          <tr>
            <td style="text-align:left; font-weight:600;">
              {{ $row['narration'] ?? 'Expense' }}
            </td>
            <td style="text-align:right; font-weight:900;">
              {{ rtrim(rtrim(number_format($row['amount'] ?? 0, 2), '0'), '.') }}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
    ...
    <table class="totals" style="width:100%;">
      ...
      <tr>
        <td class="label">Total Payment Received</td>
        <td class="value">-{{ rtrim(rtrim(number_format($voucher->total_amount ?? 0, 2), '0'), '.') }}</td>
      </tr>
      <tr>
        <td class="label">Amount After</td>
        <td class="value">
          {{ rtrim(rtrim(number_format(($previousBalance - ($voucher->total_amount ?? 0)), 2), '0'), '.') }}</td>
      </tr>
    </table>
  ...
  <script>
    function numberToWords(num) {
      ...
      function convert_hundred(n) {
        ...
        if (n > 19) { s += tens[Math.floor(n / 10)] + (n % 10 ? " " + ones[n % 10] : ""); }
        else { s += ones[n]; }
        return s.trim();
      }
